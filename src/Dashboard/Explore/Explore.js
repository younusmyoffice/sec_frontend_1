            ...
            {/* {process.env.NODE_ENV === 'development' && (
                <Box sx={{ 
                    ...
                    <Box sx={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        ...
                        {user && (
                            <Typography variant="body2">
                                <strong>User Info:</strong> {user.email || "No email"} | Role: {user.roleId || "No role"}
                            </Typography>
                        )}
                        <Typography variant="body2">
                            <strong>Login Status Check:</strong> {checkLoginStatus() ? "✅ Valid" : "❌ Invalid"}
                        </Typography>
                        <Box sx={{ display: "flex", gap: "10px", alignItems: "center" }}>
                            <LogoutButton 
                                buttonText="Test Logout"
                                buttonCss={{ backgroundColor: "#f44336", color: "white" }}
                                onLogoutSuccess={() => {
                                    console.log("Logout successful from Explore page!");
                                    alert("Logout successful! You will be redirected to login page.");
                                }}
                                onLogoutError={(error) => {
                                    console.error("Logout error from Explore page:", error);
                            ...
                            <CustomButton
                                ...
                                handleClick={() => {
                                    ...
                                    alert(`Auth Status: ${isAuthenticated ? 'Logged In' : 'Not Logged In'}\nCheck console for details.`);
                                }}
                                buttonCss={{ backgroundColor: "#2196f3", color: "white" }}
                            />
                            <CustomButton
                                label="Check Force Logout"
                                handleClick={async () => {
                                    console.log("Checking force logout status...");
                                    const result = await checkForceLogoutStatus();
                                    console.log("Force logout check result:", result);
                                    
                                    if (result.success) {
                                        if (result.forceLoggedOut) {
                                            alert(`Force Logout Status: User was force logged out!\nMessage: ${result.message}`);
                                        } else {
                                            alert(`Force Logout Status: User is still logged in\nMessage: ${result.message}`);
                                        }
                                    } else {
                                        alert(`Force Logout Check Failed: ${result.message}\nCheck console for details.`);
                                    }
                                }}
                                buttonCss={{ backgroundColor: "#ff9800", color: "white" }}
                            />
                        </Box>
                    </Box>
                </Box>
