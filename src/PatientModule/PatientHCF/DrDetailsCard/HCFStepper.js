...
export default function HCFStepper({ data }) {
    console.log("🔍 HCFStepper received data:", data);
    
    // Get hcfID from URL params since data.hcf_id might be wrong
    const { hcfID } = useParams();
    console.log("🔍 HCF ID from URL params:", hcfID);
    ...
    const [appointmentData, setAppointmentData] = React.useState({
        ...
        doctor_id: data?.hcf_id ? String(data.hcf_id) : "", // Use hcf_id field as doctor_id (as per your description)
        exam_id: data?.exam_id ? String(data.exam_id) : "",
        status: "requested",
        payment_method_nonce: null,
    });

    // Debug the corrected mapping
    console.log("🔧 Corrected ID mapping:");
    console.log("  - HCF ID (from URL):", hcfID);
    console.log("  - Doctor ID (from data.hcf_id):", data?.hcf_id);
    console.log("  - Exam ID:", data?.exam_id);
    console.log("  - Sub Exam ID:", data?.sub_exam_id);
    ...
    const FetchDoctorAvailableDates = async () => {
        ...
        try {
            // Use hcfID from URL params instead of data.hcf_id
            const resp = await axiosInstance(`/sec/patient/availableLabTestDates/${hcfID}/${data?.exam_id}`);
    ...
    const bookappointment = async (nonce_value) => {
        ...
        try {
            const appointmentPayload = { ...appointmentData, payment_method_nonce: nonce_value };
            console.log("📝 Creating HCF appointment with data:", appointmentPayload);
