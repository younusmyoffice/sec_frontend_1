...
const ClinicLogin = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Debug logging to track email discrepancy
            console.log("🔍 LOGIN DEBUG INFO:");
            console.log("Input email:", email);
            console.log("API response email:", resData?.data?.email);
            console.log("Email match:", email === resData?.data?.email);
            ...
            if (resData?.body === "INCOMPLETE_PROFILE") {
                ...
                console.log("🔑 Storing token for incomplete profile:", resData?.data?.access_token);
                localStorage.setItem("access_token", resData?.data?.access_token);
                localStorage.setItem("clinic_suid", resData?.data?.suid);
                localStorage.setItem("clinic_Email", resData?.data?.email);
                localStorage.setItem("profile", resData?.data?.profile_picture);
                Cookies.set("clinicEmail", resData?.data?.email, { expires: 7 });
             
                
                navigate("/clinicdoctorcompleteprofile");
            } else if (resData?.access_token) {
                console.log("✅ Login successful, storing data:");
                console.log("Storing email:", resData.email);
                ...
                console.log("📦 Stored in localStorage:", localStorage.getItem("clinic_Email"));
                console.log("🍪 Stored in cookies:", Cookies.get("clinicEmail"));
